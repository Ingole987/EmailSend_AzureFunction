...
namespace Email_Standalone
{
    class LogicApp
    {

        public static string PostMail(string json, ILogger log)
        {
            try
            {
        ...
        public static async Task<List<EmailModel>> DataIngetion(ILogger log)
        {
            try
            {
                ...
                {
                    await con.OpenAsync();
                    using (SqlCommand cmd = new SqlCommand("select ClientName,SourceName, SynapseTriggerId,Status, LastActivityOn from  LivevoxDownloadCallAPIStatus where LastActivityOn > dateadd(minute, -100, getdate()) and SynapseTriggerId like '%_Ingestion' and (Status like '%Completed' or datediff(hour,CreatedOn,Getdate()) > '2')", con))
                    {
                        ...
                        using (SqlDataReader dataReader = await cmd.ExecuteReaderAsync())
                        {
                            ...
                            while (dataReader.Read())
                            {
                                ...
                                emailModel.title = "Backend Integration Summary (Post Ingestion)";
                                ...
                                emailModel.pipilineName = "Deployment: Dev-Test " + emailModel.ClientName +" ("+emailModel.SourceName+")";

                                Stats newSynapseTriggerIdList = ParseStats(emailModel.SynapseTriggerId, log);

                                Stats rep = await DataIngetion2(emailModel.ClientName, emailModel.SourceName, log);
        ...
        public static async Task<Stats> DataIngetion2(string ClientName, string SourceName, ILogger log)
        {
            try
            {
